...
if ($action == 'delete')
{
    ...
    if (!$sure)
    {
        error_message("warn", "Warning", "<a href='controlpanel.php?fileaction=22&amp;action=delete&amp;staffnewsid=$staffnewsid&amp;returnto=$returnto&amp;sure=1'>Do you really want to Delete a News Item?  Click if you are sure?</a>");
    }
    ...
    sql_query("DELETE
                FROM staffnews
                WHERE id = $staffnewsid AND userid = $CURUSER[id]");

    @unlink(CACHE_DIR."staffnews.txt");
...
if ($action == 'add')
{
    ...
    @sql_query("INSERT INTO staffnews (userid, added, body)
...
if ($action == 'edit')
{
    ...
    {
        site_header("",false);
        echo("<h1>Edit A Staff News Item</h1>\n");
        echo("<form method='post' name='ednews' action='controlpanel.php?fileaction=22&amp;action=edit&amp;staffnewsid=$staffnewsid'>\n");
...
site_header("Staff News",false);
echo("<h1>Submit A Staff News Item</h1>\n");
...
echo("<form method='post' name='staffnews' action='controlpanel.php?fileaction=22&amp;action=add'>\n");
echo("<table border='1' width='100%' cellspacing='0' cellpadding='5'>\n");
...
if (mysql_num_rows($res) > 0)
{
    ...
    while ($arr = mysql_fetch_assoc($res))
    {
        ...
        echo("$added&nbsp;---&nbsp;by&nbsp;$by");
        echo(" - [<a href='controlpanel.php?fileaction=22&amp;action=edit&amp;staffnewsid=$staffnewsid'><span style='font-weight:bold;'>Edit</span></a>]");
        echo(" - [<a href='controlpanel.php?fileaction=22&amp;action=delete&amp;staffnewsid=$staffnewsid'><span style='font-weight:bold;'>Delete</span></a>]");
        echo("</td></tr></table>\n");
