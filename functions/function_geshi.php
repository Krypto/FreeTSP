...
class GeSHi
{
     ...
     * @var array
     */
    var $error_messages = array(GESHI_ERROR_NO_SUCH_LANG             => 'GeSHi could not find the language {LANGUAGE} (using path {PATH})',
     ...
     * @var array
     */
    var $lexic_permissions = array('KEYWORDS'    => array(),
    ...
    function error ()
    {
        if ($this->error)
        {
            ...
            $debug_tpl_vars = array('{LANGUAGE}' => $this->language,
    ...
    function set_header_type ($type)
    {
        ...
        if (!in_array($type, array(GESHI_HEADER_NONE,
                                   GESHI_HEADER_DIV,
                                   GESHI_HEADER_PRE,
    ...
    {
        if (in_array($case, array(GESHI_CAPS_NO_CHANGE,
                                  GESHI_CAPS_UPPER,
                                  GESHI_CAPS_LOWER))
        )
        {
    ...
    function enable_highlighting ($flag = true)
    {
        ...
        foreach ($this->lexic_permissions
                 AS
                 $key
            =>
                 $value)
    ...
    function optimize_keyword_group ($key)
    {
        ...
        {
            foreach ($this->language_data['CACHED_KEYWORD_LISTS'][$key]
                     AS
                     $rxk
                =>
                     $rxv)
    ...
    function build_parse_cache ()
    {
        ...
        if ($this->lexic_permissions['SYMBOLS'] && !empty ($this->language_data['SYMBOLS']))
        {
            ...
            // single char symbols
            foreach ($this->language_data['SYMBOLS']
                     AS
                     $key
                =>
                     $symbols)
        ...
        foreach (array_keys($this->language_data['KEYWORDS'])
                 AS
                 $key)
        {
        ...
        if ($this->lexic_permissions['NUMBERS'])
        {
            ...
            foreach ($this->language_data['NUMBERS_CACHE']
                     AS
                     $key
                =>
                     $rxdata)
    ...
    function parse_code ()
    {
                         ...
                         $delimiters)
                {
                    if (is_array($delimiters))
                    {
                                 ...
                                 $close)
                        {
                            ...
                            {
                                $matches[$dk][$open] = array('next_match'   => -1,
                                                             'dk'           => $dk,
                                                             'open'         => $open,
                    ...
                    else
                    {
                        ...
                        )
                        {
                            ...
                            {
                                $matches[$dk] = array('next_match'   => $matches_rx['start'][1],
                                                      'dk'           => $dk,
                            ...
                            {
                                $matches[$dk] = array('next_match'   => $matches_rx[1][1],
                                                      'dk'           => $dk,
        ...
        {
            foreach ($this->language_data['QUOTEMARKS']
                     AS
                     $quotemark)
            {
                if (!isset ($is_string_starter[$quotemark[0]]))
                {
                    ...
                    {
                        $is_string_starter[$quotemark[0]] = array($is_string_starter[$quotemark[0]],
                                                                  $quotemark);
                    }
                    else
                    {
            ...
            if ($highlight_part)
            {
                     ...
                     ++$i)
                {
                    ...
                    if (isset ($this->language_data['COMMENT_REGEXP']) && $next_comment_regexp_pos < $i)
                    {
                        ...
                        foreach ($this->language_data['COMMENT_REGEXP']
                                 AS
                                 $comment_key
                            =>
                                 $regexp)
                        {
                            ...
                            if (isset ($comment_regexp_cache_per_key[$comment_key]) && ($comment_regexp_cache_per_key[$comment_key]['pos'] >= $i || $comment_regexp_cache_per_key[$comment_key]['pos'] === false))
                            {
                                ...
                                if ( //This is to allow use of the offset parameter in preg_match and stay as compatible with older PHP versions as possible
                    ...
                    if ($string_started && ($i != $next_comment_regexp_pos))
                    {
                        ...
                        do
                        {
                            ...
                            if ($this->lexic_permissions['ESCAPE_CHAR'])
                            {
                                ...
                                {
                                    $next_escape_regexp_pos = $length;
                                    foreach ($this->language_data['ESCAPE_REGEXP']
                                             AS
                                             $escape_key
                                        =>
                                             $regexp)
                        ...
                        else
                        {
                            ...
                            if (!$COMMENT_MATCHED)
                            {
                                ...
                                if (!empty ($this->language_data['COMMENT_MULTI']) && $next_comment_multi_pos < $i)
                                {
                                    ...
                                    foreach ($this->language_data['COMMENT_MULTI']
                                             AS
                                             $open
                                        =>
                                             $close)
                            ...
                            if (!$COMMENT_MATCHED)
                            {
                                ...
                                if (!empty ($this->language_data['COMMENT_SINGLE']) && $next_comment_single_pos < $i)
                                {
                                    ...
                                    foreach ($this->language_data['COMMENT_SINGLE']
                                             AS
                                             $comment_key
                                        =>
                                             $comment_mark)
        ...
        //actually highlighting symbols.
        /** NOTE: memorypeak #3 */
        $endresult = str_replace(array('<SEMI>',
                                       '<PIPE>'), array(';',
                                                        '|'), $endresult);
    ...
    function handle_keyword_replace ($match)
    {
        ...
        if ($this->keyword_links)
        {
            ...
            if (isset ($this->language_data['URLS'][$k]) && $this->language_data['URLS'][$k] != '')
            {
                ...
                {
                    foreach ($this->language_data['KEYWORDS'][$k]
                             AS
                             $word)
                    {
                ...
                $before = '<|UR1|"'.str_replace(array('{FNAME}',
                                                      '{FNAMEL}',
                                                      '{FNAMEU}',
    ...
    function parse_non_string_part ($stuff_to_parse)
    {
                 ...
                 $k)
        {
            if (!isset ($this->lexic_permissions['KEYWORDS'][$k]) || $this->lexic_permissions['KEYWORDS'][$k])
            {
                ...
                for ($set = 0,
                     $set_length = count($this->language_data['CACHED_KEYWORD_LISTS'][$k]);
                     $set < $set_length;
                     ++$set)
                {
                    ...
                    $stuff_to_parse = preg_replace_callback("/$disallowed_before_local({$keywordset})(?!\<DOT\>(?:htm|php))$disallowed_after_local/$modifiers", array($this,
                                                                                                                                                                      'handle_keyword_replace'), $stuff_to_parse);
                }
            }
        }
        // Regular expressions
        foreach ($this->language_data['REGEXPS']
                 AS
                 $key
            =>
                 $regexp)
        ...
        if ($this->lexic_permissions['NUMBERS'] && preg_match('#\d#', $stuff_to_parse))
        {
            ...
            foreach ($this->language_data['NUMBERS_RXCACHE']
                     AS
                     $id
                =>
                     $regexp)
        ...
        if ($this->lexic_permissions['METHODS'] && $this->language_data['OOLANG'])
        {
            ...
            foreach ($this->language_data['OBJECT_SPLITTERS']
                     AS
                     $key
                =>
                     $splitter)
        ...
        foreach (array_keys($this->language_data['REGEXPS'])
                 AS
                 $key)
        {
            if ($this->lexic_permissions['REGEXPS'][$key])
    ...
    function load_language ($file_name)
    {
        ...
        foreach (array_keys($this->language_data['COMMENT_SINGLE'])
                 AS
                 $key)
        {
        ...
        {
            foreach ($this->language_data['PARSER_CONTROL']['ENABLE_FLAGS']
                     AS
                     $flag
                =>
                     $value)
            {
                ...
                {
                    foreach ($this->lexic_permissions[$flag]
                             AS
                             $key
                        =>
                             $val)
    ...
    function get_stylesheet ($economy_mode = true)
    {
        ...
        foreach ($this->language_data['STYLES']['KEYWORDS']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->language_data['STYLES']['ESCAPE_CHAR']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->language_data['STYLES']['BRACKETS']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->language_data['STYLES']['SYMBOLS']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->language_data['STYLES']['STRINGS']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->language_data['STYLES']['NUMBERS']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->language_data['STYLES']['METHODS']
                 AS
                 $group
            =>
                 $styles)
        ...
        // note: neglect economy_mode, empty styles are meaningless
        foreach ($this->language_data['STYLES']['SCRIPT']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->language_data['STYLES']['REGEXPS']
                 AS
                 $group
            =>
                 $styles)
        ...
        foreach ($this->highlight_extra_lines_styles
                 AS
                 $lineid
            =>
                 $linestyle)
