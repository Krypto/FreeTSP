...
* @licensehttp://gnu.org/copyleft/gpl.html GNU GPL
*
*/
//
// GeSHi Constants
...
define('GESHI_NUMBER_OCT_PREFIX_0O', 512);
//0[0-7]+
/** Number format to highlight octal numbers with a suffix of o */
define('GESHI_NUMBER_OCT_SUFFIX', 1024);
//[0-7]+[oO]
...
*
* @package   geshi
* @authorNigel McNie <nigel@geshi.org>, Benny Baumann <BenBE@omorphia.de>
* @copyright (C) 2004 - 2007 Nigel McNie, (C) 2007 - 2008 Benny Baumann
*/
class GeSHi
{
...
* @var array
*/
	var $error_messages = array(GESHI_ERROR_NO_SUCH_LANG => 'GeSHi could not find the language {LANGUAGE} (using path {PATH})', GESHI_ERROR_FILE_NOT_READABLE => 'The file specified for load_from_file was not readable', GESHI_ERROR_INVALID_HEADER_TYPE => 'The header type specified is invalid', GESHI_ERROR_INVALID_LINE_NUMBER_TYPE => 'The line number type specified is invalid');
/**
...
* @var array
*/
	var $lexic_permissions = array('KEYWORDS' => array(), 'COMMENTS' => array('MULTI' => true), 'REGEXPS' => array(), 'ESCAPE_CHAR' => true, 'BRACKETS' => true, 'SYMBOLS' => false, 'STRINGS' => true, 'NUMBERS' => true, 'METHODS' => true, 'SCRIPT' => true);
/**
...
//Put some template variables for debugging here ...
			...
			return "<br /><span style='font-weight:bold;'>GeSHi Error:</span> $msg (code {$this->error})<br />";
		}
		return false;
	}
/**
...
//Check if we got a valid header type
		if (!in_array($type, array(GESHI_HEADER_NONE, GESHI_HEADER_DIV, GESHI_HEADER_PRE, GESHI_HEADER_PRE_VALID, GESHI_HEADER_PRE_TABLE)))
		{
...
*
* @param intThe key of the keyword group to add the keyword to
...
*
* @param intThe key of the keyword group to remove the keyword from
...
*
* @param intThe key of the keyword group to create
...
*
* @param intThe key of the keyword group to remove
* @since 1.0.0
*/
...
//Decide on which style to use
			if ($style === null)
			{
//Check if we should use default style
				unset ($this->highlight_extra_lines_styles[$lines]);
			}
			else
				if ($style === false)
				{
//Check if to remove this line
...
// brackets
		if ($this->lexic_permissions['BRACKETS'])
		{
			...
			{
				$this->language_data['CACHE_BRACKET_REPLACE'] = array('<| style="' . $this->language_data['STYLES']['BRACKETS'][0] . '">&#91;|>', '<| style="' . $this->language_data['STYLES']['BRACKETS'][0] . '">&#93;|>', '<| style="' . $this->language_data['STYLES']['BRACKETS'][0] . '">&#40;|>', '<| style="' . $this->language_data['STYLES']['BRACKETS'][0] . '">&#41;|>', '<| style="' . $this->language_data['STYLES']['BRACKETS'][0] . '">&#123;|>', '<| style="' . $this->language_data['STYLES']['BRACKETS'][0] . '">&#125;|>',);
			}
			else
			{
				$this->language_data['CACHE_BRACKET_REPLACE'] = array('<| class="br0">&#91;|>', '<| class="br0">&#93;|>', '<| class="br0">&#40;|>', '<| class="br0">&#41;|>', '<| class="br0">&#123;|>', '<| class="br0">&#125;|>',);
			}
		}
...
//All this formats are matched case-insensitively!
			static $numbers_format = array(GESHI_NUMBER_INT_BASIC => '(?:(?<![0-9a-z_\.%])|(?<=\.\.))(?<![\d\.]e[+\-])([1-9]\d*?|0)(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_INT_CSTYLE => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])([1-9]\d*?|0)l(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_BIN_SUFFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])[01]+?[bB](?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_BIN_PREFIX_PERCENT => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])%[01]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_BIN_PREFIX_0B => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])0b[01]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_OCT_PREFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])0[0-7]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_OCT_PREFIX_0O => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])0o[0-7]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_OCT_SUFFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])[0-7]+?o(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_HEX_PREFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])0x[0-9a-fA-F]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_HEX_SUFFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])\d[0-9a-fA-F]*?[hH](?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_FLT_NONSCI => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])\d+?\.\d+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_FLT_NONSCI_F => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])(?:\d+?(?:\.\d*?)?|\.\d+?)f(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_FLT_SCI_SHORT => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])\.\d+?(?:e[+\-]?\d+?)?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)', GESHI_NUMBER_FLT_SCI_ZERO => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])(?:\d+?(?:\.\d*?)?|\.\d+?)(?:e[+\-]?\d+?)?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)');
...
// needed for grouping of adjacent code blocks (see below)
								'open_strlen' => strlen($open), 'close' => $close, 'close_strlen' => strlen($close),);
							}
		...
		if ($this->lexic_permissions['STRINGS'])
		{
			foreach ($this->language_data['QUOTEMARKS'] AS $quotemark)
			{
				if (!isset ($is_string_starter[$quotemark[0]]))
				{
					...
					if (is_string($is_string_starter[$quotemark[0]]))
					{
...
//This code tries to work around SF#2037598 ...
										if (function_exists('mb_substr'))
										{
											...
											if (!GESHI_PHP_PRE_433 && 'utf-8' == $this->encoding)
											{
												if (preg_match("/[\xC2-\xDF][\x80-\xBF]" . "|\xE0[\xA0-\xBF][\x80-\xBF]" . "|[\xE1-\xEC\xEE\xEF][\x80-\xBF]{2}" . "|\xED[\x80-\x9F][\x80-\xBF]" . "|\xF0[\x90-\xBF][\x80-\xBF]{2}" . "|[\xF1-\xF3][\x80-\xBF]{3}" . "|\xF4[\x80-\x8F][\x80-\xBF]{2}/s", $part, $es_char_m, null, $es_pos + 1))
												{
								...
								if ($next_escape_regexp_pos < $length && $next_escape_regexp_pos < $close_pos)
								{
...
// that languages can/cannot have multiline strings???
						...
						if ($this->lexic_permissions['STRINGS'] && $hq && $hq[0] == $char && substr($part, $i, $hq_strlen) == $hq && ($i != $next_comment_regexp_pos))
						{
...
// this comment is never matched
											...
											if (($match_i = stripos($part, $open, $i)) !== false)
											{
...
// case sensitive comments
												($this->language_data['CASE_SENSITIVE'][GESHI_COMMENTS] && ($match_i = stripos($part, $comment_mark, $i)) !== false) ||
// non case sensitive
...
/// Replace tabs with the correct number of spaces
		if (false !== strpos($result, "\t"))
		{
			$lines = explode("\n", $result);
			$result = null;
//Save memory while we process the lines individually
...
//Moved out of the look as it doesn't change within the loop
			...
			$result = implode("\n", $lines);
			unset ($lines);
//We don't need the lines separated beyond this --- free them!
		}
// Other whitespace
...
// New system: get keyword from language file to get correct case
				...
				$before = '<|UR1|"' . str_replace(array('{FNAME}', '{FNAMEL}', '{FNAMEU}', '.'), array(str_replace('+', '%20', urlencode($this->hsc($word))), str_replace('+', '%20', urlencode($this->hsc(strtolower($word)))), str_replace('+', '%20', urlencode($this->hsc(strtoupper($word)))), '<DOT>'), $this->language_data['URLS'][$k]) . '">';
				$after = '</a>';
			}
		}
...
// Highlight keywords
		...
		if (isset ($this->language_data['PARSER_CONTROL']))
		{
			...
			{
				$x = 0;
// check wether per-keyword-group parser_control is enabled
...
// get highlighted if the language has a CSS keyword in it (like CSS, for example ;))
					$stuff_to_parse = preg_replace_callback("/$disallowed_before_local({$keywordset})(?!\<DOT\>(?:htm|php))$disallowed_after_local/$modifiers", array($this, 'handle_keyword_replace'), $stuff_to_parse);
				}
			}
		}
// Regular expressions
...
// produce valid HTML when we match multiple lines
						...
						$stuff_to_parse = preg_replace_callback("/" . $regexp[GESHI_SEARCH] . "/{$regexp[GESHI_MODIFIERS]}", array($this, 'handle_multiline_regexps'), $stuff_to_parse);
...
// produce valid HTML when we match multiple lines
						$this->_hmr_key = $key;
						$stuff_to_parse = preg_replace_callback("/(" . $regexp . ")/", array($this, 'handle_multiline_regexps'), $stuff_to_parse);
						$this->_hmr_key = '';
					}
					else
					{
...
// Add class/style for regexps
		foreach (array_keys($this->language_data['REGEXPS']) AS $key)
		{
			if ($this->lexic_permissions['REGEXPS'][$key])
			{
				...
				{
					$this->_rx_key = $key;
					$stuff_to_parse = preg_replace_callback("/!REG3XP$key!(.*)\|>/U", array($this, 'handle_regexps_callback'), $stuff_to_parse);
				}
				else
				{
					if (!$this->use_classes)
...
//was not to be processed as an entity.
		';' => '<SEMI>',
// Force ; to be processed as entity
		'|' => '<PIPE>'
// Force | to be processed as entity
		);
// ENT_COMPAT set
...
*/
	...
	{
		$regex_chars = array('.', '\\', '+', '*', '?', '[', '^', ']', '$', '(', ')', '{', '}', '=', '!', '<', '>', '|', ':', $regexp_delimiter);
		sort($list);
		$regexp_list = array('');
		$num_subpatterns = 0;
		$list_key = 0;
